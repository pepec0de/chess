...
public class Chess extends JFrame {
    ...
    // Check that there is a game running
    ...
    public static AvoidingCheckTiles avoidingCheckTiles = new AvoidingCheckTiles();
    ...
    private void initValues() {        
        ...
        this.playerW = true;
        this.playerB = false;
        this.canKingCastle[0] = true;
        this.canKingCastle[1] = true;
        this.canDoLongCastling[0] = true;
        this.canDoLongCastling[1] = true;
        this.canDoShortCastling[0] = true;
        this.canDoShortCastling[1] = true;
    }
    ...
    ActionListener tileListener = new ActionListener() {
        ...
        if (!pressed) {
            ...
            if (!tilesHandler.isCheck(player)) {
                // USO DE MovingTiles | KillingTiles
                // NO ES JAQUE
                setMovingTiles();
                setKillingTiles();
            } else {
                // USO DE AvoidingCheckTiles
            ...
            if (locationX == newlocationX && locationY == newlocationY) {
                ...
                tilesHandler.setNullTiles(false); // set tiles with no figure false
                // Player turning
                if (playerW) {
                ...
                tilesHandler.setNullTiles(false);
                tilesHandler.setAllTiles(false);
                turns();
                pressed = false;
            }
    ...
    private boolean checkWin(String player) {
        switch(player) {
            case "b":
                // Check if player black wins
                for(int x = 0; x < 8; x++) {
                    for(int y = 0; y < 8; y++) {
                        // Buscar fichas blancas
                        if(tilesHandler.isWhite(x, y)) {
                            for(int ix = 0; ix < 8; ix++) {
                                for(int iy = 0; iy < 8; iy++) {
                                    // Si existe algun movimiento para evitar el jaque...
                                    if(avoidingCheckTiles.get(tilesHandler.getTile(x, y), x, y)[ix][iy]) {
                                        // Entonces no hay victoria
                                        return false;
                                    }
                                }
                            }
            ...
            case "w":
                // Check if player white wins
                for(int x = 0; x < 8; x++) {
                    for(int y = 0; y < 8; y++) {
                        // Buscar fichas negras
                        if(tilesHandler.isBlack(x, y)) {
                            for(int ix = 0; ix < 8; ix++) {
                                for(int iy = 0; iy < 8; iy++) {
                                    // Si existe algun movimiento para evitar el jaque...
                                    if(avoidingCheckTiles.get(tilesHandler.getTile(x, y), x, y)[ix][iy]) {
    ...
    public boolean[][] movingTilesget(String tile, int posx, int posy) {
        ...
        switch(tile) {
            ...
            case "wPawn":
            case "bPawn":
                bool = movingTiles.get(tile, posx, posy);
            break;                          
        }
    ...
    public void setAvoidingCheckTiles() {
        for (int x = 0; x < 8; x++) {
            for (int y = 0; y < 8; y++) {
                if (avoidingCheckTiles.get(tilesHandler.getTile(locationX, locationY), locationX, locationY)[x][y]) {
    ...
    // Unused func
    public boolean[][] avoidingCheckTilesget(String tile, int posx, int posy) {
        boolean[][] bool = new boolean[8][8];
        return bool;
    }
...
public class Chess extends JFrame {
    ...
    public static MovingTiles movingTiles = new MovingTiles();
    public static KillingTiles killingTiles = new KillingTiles();
    public static AvoidingCheckTiles blockingCheckTiles = new AvoidingCheckTiles();
